...
static int rtl_cmd_port_enable(uint8_t port, uint8_t enable)
{
...
static int rtl_port_setup(const struct config *config)
{
	size_t i;
	...
	for (i = 0; i < config->port_count; i++)
		rtl_cmd_port_enable(i, !!config->ports[i].enable);
...
static int rtl_initial_setup(const struct config *config)
{
	rtl_cmd_why_u_reset();
	...
	rtl_port_setup(config);

	return 0;
}

static int rtl_poll(const struct config *config)
{
	size_t i;
