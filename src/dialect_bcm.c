 ...
 *	1: Enable
 */
static int poe_cmd_port_enable(uint8_t port, uint8_t enable)
{
...
static int bcm_port_setup(const struct config *config)
{
	size_t i;
...
static int bcm_initial_setup(const struct config *config)
{
	poe_cmd_status();
	...
	bcm_port_setup(config);

	return 0;
}

static int bcm_poll(const struct config *config)
{
	size_t i;
